...
export const NavigationLinksPresenter = ({ className, currentPath }) => (
  <div className={className}>
    {links.map(({ pathRegExp, path, text }) => {
      // test()は正規表現(pathRegExp)と、指定された文字列(currentPath)が一致するか調べる
      // 今のページがTOPなら、TOPへはリンクがなく、お気に入りへはリンクがある
      // 今のページがお気に入りなら、TOPへのリンクがあり、お気に入りへはリンクがない
      // 今のページがその他（playerページなど）なら、TOPにもお気に入りにもリンクがある
      const isCurrentPath = pathRegExp.test(currentPath);
      const Text = <LinkText bold={!isCurrentPath}>{text}</LinkText>;
      return (
